import { createContext, useContext } from 'react';

export type LanguageCode = 'fr' | 'en';

export const TRANSLATIONS = {
  fr: {
    appName: 'Homybudget',
    settingsLabel: 'Paramètres',
    backLabel: 'Retour',
    logoutLabel: 'Déconnexion',
    budgetLabel: 'Budget',
    dashboardLabel: 'Tableau de bord',
    reportsLabel: 'Rapport',
    addNextMonth: 'Ajouter le mois suivant',
    showNextMonthLabel: 'Voir mois suivant',
    deleteMonth: 'Supprimer ce mois',
    deleteMonthConfirm: 'Cette action est irréversible.',
    deleteMonthPrompt: 'Tape SUPPRIMER pour confirmer.',
    deleteMonthConfirmToken: 'SUPPRIMER',
    cancelLabel: 'Annuler',
    confirmDeleteLabel: 'Confirmer la suppression',
    deleteMonthError: 'La suppression a échoué. Vérifie la connexion au serveur.',
    monthSelectLabel: 'Sélection du mois',
    tableLabel: 'Tableau',
    person1Label: 'Personne 1',
    person2Label: 'Personne 2',
    incomeLabel: 'Revenus',
    incomePlaceholder: 'Source de revenu',
    incomeCategoryLabel: 'Catégorie',
    autoLabel: 'Auto',
    incomeSyncOnLabel: 'Synchronisé sur les mois suivants',
    incomeSyncOffLabel: 'Non synchronisé',
    expenseSyncOnLabel: 'Synchronisé sur les mois suivants',
    expenseSyncOffLabel: 'Non synchronisé',
    totalIncomeLabel: 'Total Revenus',
    totalExpensesLabel: 'Total des Dépenses',
    availableLabel: 'Disponible',
    availableNowLabel: 'Disponible',
    monthlyBalanceLabel: 'Solde du mois',
    monthlyExpensesCardTitle: 'Dépenses mensuelles',
    monthlySpendingTitle: 'Dépenses mensuelles',
    forecastLabel: 'Prévision',
    actualLabel: 'Réel',
    inProgressLabel: 'en cours',
    monthProgressTitle: 'Progression du mois',
    actualVsForecastLabel: 'Réel vs prévision',
    leftLabel: 'restant',
    budgetedLabel: 'budgeté',
    transactionsCta: 'Transactions',
    underBudgetLabel: 'sous budget',
    overBudgetLabel: 'au-dessus',
    jointAccountCardTitle: 'Compte joint',
    jointAccountCardSubtitle: 'Solde actuel',
    transactionsLabel: 'Transactions',
    savingsCardTitle: 'Épargne',
    savingsCardSubtitle: 'Disponible pour épargne',
    topCategoriesTitle: 'Top catégories',
    topCategoriesSubtitle: 'Ce mois-ci',
    upcomingLabel: 'À venir',
    upcomingSubtitle: 'Dépenses datées',
    noDataLabel: 'Pas assez de données.',
    noUpcomingLabel: 'Aucune dépense à venir.',
    settingsExtrasTitle: 'Fonctionnalités supplémentaires',
    adminSectionTitle: 'Administration',
    fixedMoneyLabel: 'Dépenses fixes',
    freeMoneyLabel: 'Dépenses variables',
    addLabel: 'Ajouter',
    totalExpensesShortLabel: 'Dépenses totales',
    remainingToPayLabel: 'Reste à payer',
    remainingToPayTotalLabel: 'Total restant',
    validateExpenseLabel: 'Valider la dépense',
    editLabel: 'Éditer',
    closeEditLabel: 'Fermer',
    deleteLabel: 'Supprimer',
    bankAccountsTitle: 'Comptes bancaires',
    bankAccountsHint: 'Définis jusqu’à 3 comptes par personne.',
    bankAccountLabel: 'Compte bancaire',
    bankAccountNoneLabel: 'Aucun compte',
    bankAccountColorLabel: 'Couleur du compte',
    bankAccountBankLabel: 'Banque',
    bankAccountSelectPlaceholder: 'Choisir une banque',
    bankAccountCustomLabel: 'Personnalisé',
    bankAccountRegionFrance: 'France',
    bankAccountRegionUs: 'États-Unis',
    bankAccountUnassignedLabel: 'Non assigné',
    addAccountLabel: 'Ajouter un compte',
    removeAccountLabel: 'Supprimer le compte',
    accountLimitHint: '3 max / personne',
    bankAccountBaseLabel: 'Compte',
    bankAccountsSettingLabel: 'Gestion des comptes bancaires',
    bankAccountsSettingHint: 'Active ou désactive les comptes par personne.',
    bankHolidaysSettingLabel: 'Jours fériés bancaires',
    bankHolidaysSettingHint: 'JSON par année. Exemple: {"2026":["2026-01-01","2026-04-03","2026-04-06","2026-05-01","2026-12-25"]}.',
    bankHolidaysSettingApply: 'Appliquer',
    bankHolidaysSettingNote: 'Si une année est absente, le calcul automatique (TARGET2) est utilisé.',
    bankHolidaysSettingError: 'JSON invalide. Utilise un objet avec des années et des dates ISO.',
    createNextYearLabel: "Créer l'année suivante",
    createNextYearHint: "Prépare les 12 mois de l'année prochaine dès maintenant.",
    createNextYearAction: 'Créer',
    nextYearCreatedLabel: 'Année créée :',
    accountsBreakdownTitle: 'Répartition par compte',
    accountsBreakdownSubtitle: 'Ce mois-ci',
    backupSectionTitle: 'Sauvegarde',
    backupSectionSubtitle: 'Exporter / importer',
    backupSectionHint: 'Télécharge un fichier JSON de sauvegarde ou restaure une sauvegarde existante.',
    backupExportButton: 'Exporter',
    backupExportingLabel: 'Export en cours...',
    backupImportButton: 'Importer',
    backupImportingLabel: 'Import en cours...',
    backupIncludeUsersLabel: 'Inclure les utilisateurs',
    backupIncludeUsersHint: 'Si désactivé, les comptes et liens OIDC sont conservés.',
    backupImportWarningFull: 'L’import remplace toutes les données actuelles (budgets, paramètres, utilisateurs, OIDC).',
    backupImportWarningData: 'L’import remplace les budgets et paramètres mais conserve les utilisateurs.',
    backupExportSuccess: 'Sauvegarde exportée.',
    backupExportError: "L'export a échoué.",
    backupImportSuccess: 'Sauvegarde importée. Rechargement...',
    backupImportError: "L'import a échoué.",
    backupImportConfirmFull: 'Importer cette sauvegarde complète ? Cette action remplacera toutes les données actuelles.',
    backupImportConfirmData: 'Importer cette sauvegarde de données ? Les utilisateurs seront conservés.',
    backupImportInvalid: 'Fichier de sauvegarde invalide.',
    reportsIncomeYtdLabel: 'Revenus totaux (YTD)',
    reportsExpensesYtdLabel: 'Dépenses totales (YTD)',
    reportsAvgPerMonthLabel: 'Moyenne par mois',
    reportsAvgPerTransactionLabel: 'Moyenne par transaction',
    reportsNoLinkedUserLabel: "Aucun utilisateur n'est lié à ce budget.",
    reportsNoDataLabel: 'Pas de données pour cette période.',
    reportsYearLabel: 'Année',
    reportsTrendTitle: 'Évolution mensuelle',
    reportsTrendSubtitle: 'Revenus vs dépenses',
    reportsBalanceTitle: 'Solde mensuel',
    reportsBalanceSubtitle: 'Revenus - dépenses',
    reportsCalendarTitle: 'Calendrier des transactions',
    reportsAccountsCalendarTitle: 'Répartition par compte',
    reportsAccountsTotalLabel: 'Total',
    reportsTableTitle: 'Détails par mois',
    reportsTableMonthLabel: 'Mois',
    reportsTableIncomeLabel: 'Revenus',
    reportsTableExpensesLabel: 'Dépenses',
    reportsTableBalanceLabel: 'Solde',
    reportsTableTransactionsLabel: 'Transactions',
    undoLabel: 'Annuler',
    undoDeleteLabel: 'Suppression effectuée.',
    undoEditLabel: 'Modification enregistrée.',
    moveUpLabel: 'Monter la ligne',
    moveDownLabel: 'Descendre la ligne',
    dragHandleLabel: 'Déplacer la ligne',
    syncFutureConfirmLabel: 'Des différences ont été détectées sur cette ligne dans les mois suivants. Synchroniser les mois suivants ?',
    addRowLabel: 'Ajouter une ligne',
    installmentLabel: 'Paiement échelonné',
    expenseLinkLabel: 'Lier aux mois suivants',
    startLabel: 'Début',
    jointAccountTitle: 'Compte joint - Épargne commune',
    depositLabel: 'Versement',
    expenseLabel: 'Dépense',
    initialBalanceLabel: 'Solde initial',
    currentBalanceLabel: 'Solde actuel',
    dateLabel: 'Date',
    clearDateLabel: 'Effacer la date',
    typeLabel: 'Type',
    descriptionLabel: 'Description',
    amountLabel: 'Montant',
    personLabel: 'Personne',
    depositOptionLabel: '+ Versement',
    expenseOptionLabel: '- Dépense',
    noOtherPaletteLabel: 'Aucune autre palette',
    accountMenuLabel: 'Menu du compte',
    loginTitle: 'Connexion',
    loginUsernameLabel: 'Identifiant',
    loginPasswordLabel: 'Mot de passe',
    showPasswordLabel: 'Afficher le mot de passe',
    hidePasswordLabel: 'Masquer le mot de passe',
    rememberMeLabel: 'Rester connecté',
    loginButton: 'Se connecter',
    loginLoading: 'Connexion...',
    loginWithProviderLabel: 'Se connecter avec',
    oidcUnlinkedError: "Ce compte n'est pas lié. Connecte-toi avec ton mot de passe puis lie-le dans les paramètres.",
    oidcInactiveError: "Ce compte est désactivé.",
    oidcFailedError: "Connexion externe impossible.",
    oidcExpiredError: 'Connexion externe expirée.',
    oidcInvalidError: 'Connexion externe invalide.',
    oidcLinkSuccess: 'Compte externe lié.',
    oidcLinkConflictError: 'Ce compte externe est déjà lié.',
    oidcLinkTitle: 'Lier un compte externe',
    oidcLinkHint: 'Associe ton compte à un fournisseur externe.',
    oidcLinkButton: 'Lier avec',
    oidcLinking: 'Liaison...',
    oidcLinkError: 'Impossible de lier le compte externe.',
    onboardingTitle: 'Première installation',
    onboardingStepLabel: 'Étape',
    onboardingAdminUsernameLabel: 'Identifiant admin',
    onboardingAdminPasswordLabel: 'Mot de passe admin',
    onboardingAdminConfirmLabel: 'Confirmer',
    onboardingCreateAdmin: 'Créer admin',
    onboardingCreatingAdmin: 'Création...',
    onboardingBootstrapError: "Impossible de créer l'admin.",
    onboardingLanguageTitle: 'Choisir la langue',
    onboardingThemeTitle: 'Mode clair ou sombre',
    onboardingModeTitle: 'Mode solo ou duo',
    onboardingSoloLabel: 'Solo',
    onboardingDuoLabel: 'Duo',
    onboardingNamesTitle: 'Noms des personnes',
    onboardingSecondUserTitle: 'Compte de la seconde personne',
    onboardingSecondUserHint: 'Ce compte pourra se connecter et suivre son budget.',
    onboardingPerson2Required: 'Veuillez renseigner la seconde personne.',
    onboardingPerson1Placeholder: 'Personne 1',
    onboardingPerson2Placeholder: 'Personne 2',
    onboardingNext: 'Suivant',
    onboardingBack: 'Retour',
    onboardingFinish: 'Terminer',
    profileTitle: 'Profil',
    profileMenuLabel: 'Profil utilisateur',
    userLabel: 'Utilisateur',
    roleLabel: 'Rôle',
    profileImageLabel: 'Image de profil',
    profileImageHint: "Colle l'URL d'une image (PNG/JPG).",
    profileImageUrlPlaceholder: 'https://exemple.com/avatar.png',
    profileImageSaveButton: 'Mettre à jour',
    profileImageSaving: 'Mise à jour...',
    profileImageUpdated: 'Image mise à jour.',
    profileImageError: "Impossible de mettre à jour l'image.",
    profileImageUploadButton: 'Uploader',
    profileImageUploadLoading: 'Upload...',
    profileImageUploadSuccess: 'Image envoyée.',
    profileImageUploadError: "Impossible d'envoyer l'image.",
    settingsSectionTitle: 'Paramètres',
    sortExpensesLabel: 'Tri des dépenses par coût',
    fixedFreeLabel: 'Dépenses fixes et variables',
    sidebarMonthsSettingLabel: 'Liste des mois dans le volet',
    sidebarMonthsSettingHint: 'Affiche tous les mois de l’année dans la barre latérale (desktop).',
    budgetWidgetsSettingLabel: 'Widgets du budget',
    budgetWidgetsSettingHint: 'Afficher les widgets à gauche dans le budget.',
    jointAccountSettingLabel: 'Compte joint',
    jointAccountSettingHint: 'Afficher ou masquer le compte joint',
    soloModeSettingLabel: 'Mode solo',
    soloModeSettingHint: 'Masquer la seconde personne',
    personLinkSectionTitle: 'Association des personnes',
    personLinkSectionHint: 'Choisis quel utilisateur correspond à chaque personne.',
    unassignedLabel: 'Aucun',
    activeLabel: 'Actif',
    inactiveLabel: 'Inactif',
    defaultModeLabel: 'Mode par défaut',
    currencyLabel: 'Devise',
    currencyEuroLabel: 'Euro (EUR)',
    currencyDollarLabel: 'Dollar (USD)',
    sessionDurationLabel: 'Durée de session',
    sessionDurationHint: 'Déconnexion automatique entre 1 et 24 heures.',
    oidcSectionTitle: 'Connexion externe (OIDC)',
    oidcEnabledLabel: 'Activer OIDC',
    oidcProviderLabel: 'Nom du provider',
    oidcIssuerLabel: 'Issuer URL',
    oidcClientIdLabel: 'Client ID',
    oidcClientSecretLabel: 'Client secret',
    oidcRedirectUriLabel: 'Redirect URI',
    themeToggleLabel: 'Mode clair/sombre',
    lightLabel: 'Clair',
    darkLabel: 'Sombre',
    languageLabel: 'Langue',
    frenchLabel: 'Français',
    englishLabel: 'English',
    moreSettingsSoon: 'Autres paramètres bientôt disponibles.',
    comingSoonLabel: 'Bientôt disponible.',
    updateAvailableLabel: 'Mise à jour dispo',
    offlineLabel: 'Hors ligne',
    syncPendingLabel: 'Sync en attente',
    syncAutoLocalLabel: 'Sync auto : local',
    syncAutoServerLabel: 'Sync auto : serveur',
    changePasswordTitle: 'Changer le mot de passe',
    currentPasswordPlaceholder: 'Mot de passe actuel',
    newPasswordPlaceholder: 'Nouveau mot de passe',
    confirmPasswordPlaceholder: 'Confirmer',
    updateButton: 'Mettre à jour',
    updatingButton: 'Mise à jour...',
    userManagementTitle: 'Gestion des utilisateurs',
    refreshButton: 'Rafraîchir',
    createUserButton: 'Créer un utilisateur',
    creatingUserButton: 'Création...',
    resetTokenTitle: 'Token de réinitialisation',
    expiresOnLabel: 'Expire le',
    loadingUsers: 'Chargement des utilisateurs...',
    noUsers: 'Aucun utilisateur trouvé.',
    accountLabel: 'Compte',
    statusLabel: 'Statut',
    lastLoginLabel: 'Dernière connexion',
    actionsLabel: 'Actions',
    blockedLabel: 'Bloqué',
    resetButton: 'Réinitialiser',
    createUserUsernamePlaceholder: 'Identifiant',
    createUserDisplayNamePlaceholder: 'Nom affiché',
    createUserPasswordPlaceholder: 'Mot de passe',
    createUserConfirmPlaceholder: 'Confirmer',
    roleUserLabel: 'Utilisateur',
    roleAdminLabel: 'Admin',
    passwordRequiredError: 'Veuillez saisir votre mot de passe actuel et le nouveau.',
    passwordMismatchError: 'Les mots de passe ne correspondent pas.',
    passwordUpdatedSuccess: 'Mot de passe mis à jour.',
    passwordChangeError: 'Impossible de changer le mot de passe.',
    userLoadError: 'Impossible de charger les utilisateurs.',
    userCreateRequiredError: 'Identifiant et mot de passe requis.',
    userCreateMismatchError: 'Les mots de passe ne correspondent pas.',
    userCreateSuccess: 'Utilisateur créé.',
    userCreateError: "Impossible de créer l'utilisateur.",
    displayNameUpdateError: "Impossible de mettre à jour le nom affiché.",
    userRoleUpdateError: 'Impossible de mettre à jour le rôle.',
    userStatusUpdateError: 'Impossible de mettre a jour le statut.',
    resetTokenError: 'Impossible de créer le token de reset.',
    monthUnavailableError: 'Mois indisponible. Crée le mois ou choisis un mois existant.',
    sessionExpiredError: 'Session expirée. Merci de vous reconnecter.',
    authRequiredError: 'Identifiants requis.',
    authInvalidError: 'Identifiants invalides.',
    authServerError: 'Impossible de se connecter au serveur.',
    expenseWizardTitle: 'Ajouter une dépense',
    expenseWizardEditTitle: 'Modifier cette dépense',
    jointWizardTitle: 'Ajouter une transaction',
    jointWizardEditTitle: 'Modifier cette transaction',
    jointDeleteSuccess: 'Transaction supprimée.',
    expenseNameLabel: 'Nom',
    categoryLabel: 'Catégorie',
    categoryAutoLabel: 'Auto',
    newIncomeSourceLabel: 'Nouvelle source',
    newFixedExpenseLabel: 'Nouvelle dépense',
    newCategoryLabel: 'Nouvelle catégorie',
    newDepositDescription: 'Nouveau versement',
    newExpenseDescription: 'Nouvelle dépense'
  },
  en: {
    appName: 'Homybudget',
    settingsLabel: 'Settings',
    backLabel: 'Back',
    logoutLabel: 'Logout',
    budgetLabel: 'Budget',
    dashboardLabel: 'Dashboard',
    reportsLabel: 'Reports',
    addNextMonth: 'Add next month',
    showNextMonthLabel: 'Show next month',
    deleteMonth: 'Delete this month',
    deleteMonthConfirm: 'This action is irreversible.',
    deleteMonthPrompt: 'Type DELETE to confirm.',
    deleteMonthConfirmToken: 'DELETE',
    cancelLabel: 'Cancel',
    confirmDeleteLabel: 'Confirm delete',
    deleteMonthError: 'Delete failed. Check the server connection.',
    monthSelectLabel: 'Select month',
    tableLabel: 'Table',
    person1Label: 'Person 1',
    person2Label: 'Person 2',
    incomeLabel: 'Income',
    incomePlaceholder: 'Income source',
    incomeCategoryLabel: 'Category',
    autoLabel: 'Auto',
    incomeSyncOnLabel: 'Syncs to future months',
    incomeSyncOffLabel: 'Not synced',
    expenseSyncOnLabel: 'Syncs to future months',
    expenseSyncOffLabel: 'Not synced',
    totalIncomeLabel: 'Total Income',
    totalExpensesLabel: 'Total Expenses',
    availableLabel: 'Available',
    availableNowLabel: 'Available',
    monthlyBalanceLabel: 'Monthly balance',
    monthlyExpensesCardTitle: 'Monthly expenses',
    monthlySpendingTitle: 'Monthly spending',
    forecastLabel: 'Forecast',
    actualLabel: 'Actual',
    inProgressLabel: 'in progress',
    monthProgressTitle: 'Monthly progress',
    actualVsForecastLabel: 'Actual vs forecast',
    leftLabel: 'left',
    budgetedLabel: 'budgeted',
    transactionsCta: 'Transactions',
    underBudgetLabel: 'under',
    overBudgetLabel: 'over',
    jointAccountCardTitle: 'Joint account',
    jointAccountCardSubtitle: 'Current balance',
    transactionsLabel: 'Transactions',
    savingsCardTitle: 'Savings',
    savingsCardSubtitle: 'Available for savings',
    topCategoriesTitle: 'Top categories',
    topCategoriesSubtitle: 'This month',
    upcomingLabel: 'Upcoming',
    upcomingSubtitle: 'Dated expenses',
    noDataLabel: 'Not enough data.',
    noUpcomingLabel: 'No upcoming expenses.',
    settingsExtrasTitle: 'Extra features',
    adminSectionTitle: 'Administration',
    fixedMoneyLabel: 'Fixed expenses',
    freeMoneyLabel: 'Variable expenses',
    addLabel: 'Add',
    totalExpensesShortLabel: 'Total expenses',
    remainingToPayLabel: 'Remaining to pay',
    remainingToPayTotalLabel: 'Remaining total',
    validateExpenseLabel: 'Mark expense',
    editLabel: 'Edit',
    closeEditLabel: 'Close',
    deleteLabel: 'Delete',
    bankAccountsTitle: 'Bank accounts',
    bankAccountsHint: 'Define up to 3 accounts per person.',
    bankAccountLabel: 'Bank account',
    bankAccountNoneLabel: 'No account',
    bankAccountColorLabel: 'Account color',
    bankAccountBankLabel: 'Bank',
    bankAccountSelectPlaceholder: 'Choose a bank',
    bankAccountCustomLabel: 'Custom',
    bankAccountRegionFrance: 'France',
    bankAccountRegionUs: 'United States',
    bankAccountUnassignedLabel: 'Unassigned',
    addAccountLabel: 'Add account',
    removeAccountLabel: 'Remove account',
    accountLimitHint: '3 max / person',
    bankAccountBaseLabel: 'Account',
    bankAccountsSettingLabel: 'Bank account management',
    bankAccountsSettingHint: 'Enable or disable accounts per person.',
    bankHolidaysSettingLabel: 'Bank holidays',
    bankHolidaysSettingHint: 'JSON per year. Example: {"2026":["2026-01-01","2026-04-03","2026-04-06","2026-05-01","2026-12-25"]}.',
    bankHolidaysSettingApply: 'Apply',
    bankHolidaysSettingNote: 'If a year is missing, automatic TARGET2 calculation is used.',
    bankHolidaysSettingError: 'Invalid JSON. Use a year-to-ISO-date array mapping.',
    createNextYearLabel: 'Create next year',
    createNextYearHint: 'Prepare the 12 months for next year now.',
    createNextYearAction: 'Create',
    nextYearCreatedLabel: 'Year created:',
    accountsBreakdownTitle: 'Account breakdown',
    accountsBreakdownSubtitle: 'This month',
    backupSectionTitle: 'Backup',
    backupSectionSubtitle: 'Export / import',
    backupSectionHint: 'Download a JSON backup file or restore an existing backup.',
    backupExportButton: 'Export',
    backupExportingLabel: 'Exporting...',
    backupImportButton: 'Import',
    backupImportingLabel: 'Importing...',
    backupIncludeUsersLabel: 'Include users',
    backupIncludeUsersHint: 'When disabled, accounts and OIDC links are kept.',
    backupImportWarningFull: 'Import replaces all current data (budgets, settings, users, OIDC).',
    backupImportWarningData: 'Import replaces budgets and settings but keeps users.',
    backupExportSuccess: 'Backup exported.',
    backupExportError: 'Export failed.',
    backupImportSuccess: 'Backup imported. Reloading...',
    backupImportError: 'Import failed.',
    backupImportConfirmFull: 'Import this full backup? This will replace all current data.',
    backupImportConfirmData: 'Import this data backup? Users will be kept.',
    backupImportInvalid: 'Invalid backup file.',
    reportsIncomeYtdLabel: 'Total Income (YTD)',
    reportsExpensesYtdLabel: 'Total Expenses (YTD)',
    reportsAvgPerMonthLabel: 'Avg Per Month',
    reportsAvgPerTransactionLabel: 'Avg Per Transaction',
    reportsNoLinkedUserLabel: 'No user is linked to this budget.',
    reportsNoDataLabel: 'No data for this period.',
    reportsYearLabel: 'Year',
    reportsTrendTitle: 'Monthly trend',
    reportsTrendSubtitle: 'Income vs expenses',
    reportsBalanceTitle: 'Monthly balance',
    reportsBalanceSubtitle: 'Income - expenses',
    reportsCalendarTitle: 'Transaction calendar',
    reportsAccountsCalendarTitle: 'Account distribution',
    reportsAccountsTotalLabel: 'Total',
    reportsTableTitle: 'Monthly details',
    reportsTableMonthLabel: 'Month',
    reportsTableIncomeLabel: 'Income',
    reportsTableExpensesLabel: 'Expenses',
    reportsTableBalanceLabel: 'Balance',
    reportsTableTransactionsLabel: 'Transactions',
    undoLabel: 'Undo',
    undoDeleteLabel: 'Item deleted.',
    undoEditLabel: 'Edit saved.',
    moveUpLabel: 'Move up',
    moveDownLabel: 'Move down',
    dragHandleLabel: 'Drag to reorder',
    syncFutureConfirmLabel: 'Differences were detected for this line in later months. Sync future months?',
    addRowLabel: 'Add row',
    installmentLabel: 'Installment',
    expenseLinkLabel: 'Link future months',
    startLabel: 'Start',
    jointAccountTitle: 'Joint Account - Shared Savings',
    depositLabel: 'Deposit',
    expenseLabel: 'Expense',
    initialBalanceLabel: 'Starting balance',
    currentBalanceLabel: 'Current balance',
    dateLabel: 'Date',
    clearDateLabel: 'Clear date',
    typeLabel: 'Type',
    descriptionLabel: 'Description',
    amountLabel: 'Amount',
    personLabel: 'Person',
    depositOptionLabel: '+ Deposit',
    expenseOptionLabel: '- Expense',
    noOtherPaletteLabel: 'No other palette',
    accountMenuLabel: 'Account menu',
    loginTitle: 'Sign in',
    loginUsernameLabel: 'Username',
    loginPasswordLabel: 'Password',
    showPasswordLabel: 'Show password',
    hidePasswordLabel: 'Hide password',
    rememberMeLabel: 'Stay signed in',
    loginButton: 'Sign in',
    loginLoading: 'Signing in...',
    loginWithProviderLabel: 'Sign in with',
    oidcUnlinkedError: 'This account is not linked. Sign in with password and link it in settings.',
    oidcInactiveError: 'This account is disabled.',
    oidcFailedError: 'External login failed.',
    oidcExpiredError: 'External login expired.',
    oidcInvalidError: 'External login is invalid.',
    oidcLinkSuccess: 'External account linked.',
    oidcLinkConflictError: 'This external account is already linked.',
    oidcLinkTitle: 'Link external login',
    oidcLinkHint: 'Associate your account with an external provider.',
    oidcLinkButton: 'Link with',
    oidcLinking: 'Linking...',
    oidcLinkError: 'Unable to link external account.',
    onboardingTitle: 'First setup',
    onboardingStepLabel: 'Step',
    onboardingAdminUsernameLabel: 'Admin username',
    onboardingAdminPasswordLabel: 'Admin password',
    onboardingAdminConfirmLabel: 'Confirm',
    onboardingCreateAdmin: 'Create admin',
    onboardingCreatingAdmin: 'Creating...',
    onboardingBootstrapError: 'Unable to create admin.',
    onboardingLanguageTitle: 'Choose language',
    onboardingThemeTitle: 'Light or dark',
    onboardingModeTitle: 'Solo or duo',
    onboardingSoloLabel: 'Solo',
    onboardingDuoLabel: 'Duo',
    onboardingNamesTitle: 'People names',
    onboardingSecondUserTitle: 'Second person account',
    onboardingSecondUserHint: 'This account can sign in and track their budget.',
    onboardingPerson2Required: 'Please enter the second person.',
    onboardingPerson1Placeholder: 'Person 1',
    onboardingPerson2Placeholder: 'Person 2',
    onboardingNext: 'Next',
    onboardingBack: 'Back',
    onboardingFinish: 'Finish',
    profileTitle: 'Profile',
    profileMenuLabel: 'User profile',
    userLabel: 'User',
    roleLabel: 'Role',
    profileImageLabel: 'Profile image',
    profileImageHint: 'Paste an image URL (PNG/JPG).',
    profileImageUrlPlaceholder: 'https://example.com/avatar.png',
    profileImageSaveButton: 'Update',
    profileImageSaving: 'Updating...',
    profileImageUpdated: 'Image updated.',
    profileImageError: 'Unable to update image.',
    profileImageUploadButton: 'Upload',
    profileImageUploadLoading: 'Uploading...',
    profileImageUploadSuccess: 'Image uploaded.',
    profileImageUploadError: 'Unable to upload image.',
    settingsSectionTitle: 'Preferences',
    sortExpensesLabel: 'Sort expenses by cost',
    fixedFreeLabel: 'Fixed and variable expenses',
    sidebarMonthsSettingLabel: 'Sidebar month list',
    sidebarMonthsSettingHint: 'Show all months of the year in the desktop sidebar.',
    budgetWidgetsSettingLabel: 'Budget widgets',
    budgetWidgetsSettingHint: 'Show the left-side widgets in the budget view.',
    jointAccountSettingLabel: 'Joint account',
    jointAccountSettingHint: 'Show or hide the joint account',
    soloModeSettingLabel: 'Solo mode',
    soloModeSettingHint: 'Hide the second person',
    personLinkSectionTitle: 'People mapping',
    personLinkSectionHint: 'Choose which user is Person 1 and Person 2.',
    unassignedLabel: 'None',
    activeLabel: 'Active',
    inactiveLabel: 'Inactive',
    defaultModeLabel: 'Default theme',
    currencyLabel: 'Currency',
    currencyEuroLabel: 'Euro (EUR)',
    currencyDollarLabel: 'Dollar (USD)',
    sessionDurationLabel: 'Session duration',
    sessionDurationHint: 'Auto sign-out between 1 and 24 hours.',
    oidcSectionTitle: 'External login (OIDC)',
    oidcEnabledLabel: 'Enable OIDC',
    oidcProviderLabel: 'Provider name',
    oidcIssuerLabel: 'Issuer URL',
    oidcClientIdLabel: 'Client ID',
    oidcClientSecretLabel: 'Client secret',
    oidcRedirectUriLabel: 'Redirect URI',
    themeToggleLabel: 'Light/Dark mode',
    lightLabel: 'Light',
    darkLabel: 'Dark',
    languageLabel: 'Language',
    frenchLabel: 'French',
    englishLabel: 'English',
    moreSettingsSoon: 'More settings coming soon.',
    comingSoonLabel: 'Coming soon.',
    updateAvailableLabel: 'update available',
    offlineLabel: 'Offline',
    syncPendingLabel: 'Sync pending',
    syncAutoLocalLabel: 'Auto sync: local',
    syncAutoServerLabel: 'Auto sync: server',
    changePasswordTitle: 'Change password',
    currentPasswordPlaceholder: 'Current password',
    newPasswordPlaceholder: 'New password',
    confirmPasswordPlaceholder: 'Confirm',
    updateButton: 'Update',
    updatingButton: 'Updating...',
    userManagementTitle: 'User management',
    refreshButton: 'Refresh',
    createUserButton: 'Create user',
    creatingUserButton: 'Creating...',
    resetTokenTitle: 'Reset token',
    expiresOnLabel: 'Expires on',
    loadingUsers: 'Loading users...',
    noUsers: 'No users found.',
    accountLabel: 'Account',
    statusLabel: 'Status',
    lastLoginLabel: 'Last login',
    actionsLabel: 'Actions',
    blockedLabel: 'Blocked',
    resetButton: 'Reset',
    createUserUsernamePlaceholder: 'Username',
    createUserDisplayNamePlaceholder: 'Display name',
    createUserPasswordPlaceholder: 'Password',
    createUserConfirmPlaceholder: 'Confirm',
    roleUserLabel: 'User',
    roleAdminLabel: 'Admin',
    passwordRequiredError: 'Please enter your current and new password.',
    passwordMismatchError: 'Passwords do not match.',
    passwordUpdatedSuccess: 'Password updated.',
    passwordChangeError: 'Unable to change password.',
    userLoadError: 'Unable to load users.',
    userCreateRequiredError: 'Username and password required.',
    userCreateMismatchError: 'Passwords do not match.',
    userCreateSuccess: 'User created.',
    userCreateError: 'Unable to create user.',
    displayNameUpdateError: 'Unable to update display name.',
    userRoleUpdateError: 'Unable to update role.',
    userStatusUpdateError: 'Unable to update status.',
    resetTokenError: 'Unable to create reset token.',
    monthUnavailableError: 'Month unavailable. Create or choose an existing month.',
    sessionExpiredError: 'Session expired. Please sign in again.',
    authRequiredError: 'Username and password required.',
    authInvalidError: 'Invalid credentials.',
    authServerError: 'Unable to connect to server.',
    expenseWizardTitle: 'Add expense',
    expenseWizardEditTitle: 'Edit expense',
    jointWizardTitle: 'Add transaction',
    jointWizardEditTitle: 'Edit transaction',
    jointDeleteSuccess: 'Transaction deleted.',
    expenseNameLabel: 'Name',
    categoryLabel: 'Category',
    categoryAutoLabel: 'Auto',
    newIncomeSourceLabel: 'New source',
    newFixedExpenseLabel: 'New expense',
    newCategoryLabel: 'New category',
    newDepositDescription: 'New deposit',
    newExpenseDescription: 'New expense'
  }
} as const;

export type TranslationKey = keyof typeof TRANSLATIONS.fr;

export const MONTH_LABELS: Record<LanguageCode, string[]> = {
  fr: [
    'Janvier', 'Fevrier', 'Mars', 'Avril', 'Mai', 'Juin',
    'Juillet', 'Aout', 'Septembre', 'Octobre', 'Novembre', 'Decembre'
  ],
  en: [
    'January', 'February', 'March', 'April', 'May', 'June',
    'July', 'August', 'September', 'October', 'November', 'December'
  ]
};

export const createTranslator = (language: LanguageCode) => {
  const fallback = TRANSLATIONS.fr;
  const dictionary = TRANSLATIONS[language] ?? fallback;
  return (key: TranslationKey) => dictionary[key] ?? fallback[key] ?? key;
};

export const TranslationContext = createContext<{ t: (key: TranslationKey) => string; language: LanguageCode }>({
  t: (key) => TRANSLATIONS.fr[key] ?? key,
  language: 'fr'
});

export const useTranslation = () => useContext(TranslationContext);
